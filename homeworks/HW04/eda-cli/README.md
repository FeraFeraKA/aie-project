...
- возвращает флаги из `compute_quality_flags` в формате JSON.
...
- `flags` - флаги качества CSV-файла из метода compute_quality_flags.

---

### 6. `GET /metrics` - простая статистика работы сервиса

Простейшая статистика сервиса (количество запросов, средняя задержка, были ли последние данные нормальны для модели).
...
**Ожидаемый ответ `200 OK` (JSON, пример):**

```json
{
  "total_requests": 4,
  "avg_latency_ms": 5.884700000024168,
